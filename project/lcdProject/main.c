...
// 读取显示图片
#if defined (IMAGE)
    ReadPicture* picture1 = ReadPict(PIC_NAME1);
    ReadPicture* picture2 = ReadPict(PIC_NAME2);
    if (NULL == picture1 || NULL == picture2) {
        printf("读取图片失败!\n");
        goto draw;
    }
    int range = 0;
    while (1) {
        printf("\n双缓冲测试轮询测试中...\n");
        ImageShow(lcd, picture1, 50, 0, range);
        range = 1;
        sleep(3);
        ImageShow(lcd, picture2, 500, 0, range);
        range = 0;
        sleep(1);
    }
    FreePict(picture1);
